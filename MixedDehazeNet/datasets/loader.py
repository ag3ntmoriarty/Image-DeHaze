...
class PairLoader(Dataset):
	...
	def __getitem__(self, idx):
		...
		img_name = self.img_names[idx][:4]
		source_img = read_img(os.path.join(self.root_dir, 'hazy', self.img_names[idx])) * 2 - 1
		target_img = read_img(os.path.join(self.root_dir, 'gt', img_name + '.png')) * 2 - 1
...
class LiveLoader():
	...
	def __getitem__(self, idx):
		...
		return {'img': hwc_to_chw(img), 'filename': 'livefeed.png'}