...
#         self.k_map = nn.ModuleList([nn.Linear(dim, dim ) for _ in range(self.n_heads)])
#         self.v_map = nn.ModuleList([nn.Linear(dim, dim ) for _ in range(self.n_heads)])
#         self.q_map = nn.ModuleList([nn.Linear(dim, dim ) for _ in range(self.n_heads)])
...
#             result.append(torch.unsqueeze(attention, dim=0))
...
class MixStructureBlock(nn.Module):
    def __init__(self, dim):
        ...
        self.conv1 = nn.Conv2d(dim, dim, kernel_size=1)#pw conv
